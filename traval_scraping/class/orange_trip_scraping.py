...
import time
from gloria_trip_scrapping import gloria
class orange(gloria):
    def test(self):
        home_p_source=[]

        for i in range(1,4):
            page_link="https://www.tripresso.com/agency/ORANGE#pageNum={p}".format(p=i)
        ...
        for i in home_p_source:
            all_link=[]
            a_type_link=re.findall('''\"\/detail\?tour_key=ORANGE-\w*\&amp;group_code=\w*\"''',i)
            b_type_link=re.findall('''\"\/detail\?tour_key=ORANGE-\w*-\w*\&amp;group_code=\w*\"''',i)
        ...
        for i in clr_all_link:
            ...
            all_complete_link.append(complete_link)   
